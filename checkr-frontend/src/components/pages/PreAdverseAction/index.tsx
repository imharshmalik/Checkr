        ...
        setTableData(
          candidatesData.map((candidate: any) => {
            const adverseAction = actionsData.find(
              (action: any) => action.candidateId === candidate.id
            );
            return {
              id: candidate.id,
              name: candidate.name,
              status: adverseAction.actionStatus?.toString().toUpperCase(),
              pre_notice_date: dateDisplay(
                adverseAction.preNoticeDate.toString()
              ),
              post_notice_date: dateDisplay(
                adverseAction.postNoticeDate.toString()
              ),
            };
          })
        );
